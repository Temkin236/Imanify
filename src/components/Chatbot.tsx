        ...
        {messages.map((msg, i) => (
          ...
          >
            <div className={`flex gap-4 max-w-[92%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-12 h-12 rounded-[1.5rem] flex items-center justify-center shrink-0 shadow-xl ${msg.role === 'user' ? 'bg-gold-500 text-islamic-green-950' : 'bg-islamic-green-900 border border-white/10'}`}>
                {msg.role === 'user' ? <User size={24} /> : <Bot size={24} />}
              </div>
              <div className={`p-6 rounded-[2.5rem] text-base leading-relaxed shadow-2xl relative ${msg.role === 'user' ? 'bg-gold-500 text-islamic-green-950 font-bold rounded-tr-none' : 'bg-white/5 border border-white/5 rounded-tl-none text-white/95'}`}>
                {msg.content}
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="flex gap-4">
              ...
              <div className="bg-white/5 border border-white/5 p-5 rounded-[2rem] rounded-tl-none flex gap-1.5 items-center">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
              {moodSuggestions.map((mood) => (
                <button
                  ...
                  className={`whitespace-nowrap px-6 py-4 rounded-[2rem] border border-white/5 flex items-center gap-3 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-xl ${mood.color}`}
                >
